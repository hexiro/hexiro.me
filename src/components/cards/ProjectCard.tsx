...
import { MotionCard } from "@/components/ui/Cards";
...
export function ProjectCard({ className, project }: ProjectCardProps) {
    ...
    const setTopicFilter = useSetAtom(topicFilterAtom);

    return (
        <MotionCard
            layout
            isHoverable={false}
            className={twMerge("w-full pr-4 md:w-[calc(50%-1rem)]", className)}
        >
                ...
                <ul className="flex h-[26px] flex-row flex-wrap gap-x-2 overflow-hidden leading-none">
                    {topics.map(({ name }) => (
                        <li key={name}>
                            <button
                                type="button"
                                className="hover:text-white/65 relative z-10 flex flex-row items-center rounded-md border-2 border-solid border-white/10 bg-background-light-accent px-2 py-1 font-sans text-sm font-bold text-white/50 transition-colors hover:bg-background-accent"
                                onClick={() => setTopicFilter(name)}
                            >
                                {name}
                            </button>
                        </li>
